...
echo "<div style = 'font-size:50px; color:white; position: absolute; left:40%; top:30%;'> Lista Soferi </div>";
...
$queryProduseiesire = "SELECT nume, telefon, id_soferi, oras FROM soferi WHERE NOT id_soferi = 2  ";
$rezultatqueryIesire = mysqli_query($conexiune, $queryProduseiesire);
while($queryiesire = mysqli_fetch_assoc($rezultatqueryIesire)) {
   $numeSofer = $queryiesire['nume'];
   $telefonSofer = $queryiesire['telefon'];
   $id_soferi = $queryiesire['id_soferi'];
   $oras = $queryiesire['oras'];
    ...
    <td><a href ='editareSoferi.php?GetIDSoferi=$id_soferi' style = 'color:blue;'>Editeaza</a> </td>
