...
echo "<div style = 'font-size:50px; color:white; position: absolute; left:40%; top:30%;'> Top 10 produse vandute </div>";
...
$queryProduseiesire = "SELECT SUM(c.cantitate) AS totalCantitate, SUM(c.cost_total) AS totalCost, p.denumire, p.pret FROM produse as p INNER JOIN produse_comenzi as c ON p.id_produse= c.id_produse GROUP BY p.id_produse ORDER BY SUM(c.cantitate) DESC LIMIT 10 ;";
...
while($queryiesire = mysqli_fetch_assoc($rezultatqueryIesire)) {
    ...
    $total_vandute = $queryiesire['totalCantitate'];
    
    $incasari_totale_produs = $queryiesire['totalCost'];
