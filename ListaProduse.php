...
$queryProduseiesire = "SELECT p.id_produse, p.denumire, p.pret, p.ingrediente, p.gramaj, p.imagini_produse, p.statusProdus, c.categorie FROM produse as p INNER JOIN categorie_produse as c ON c.id_categorie = p.id_categorie ORDER BY denumire;";
...
while($queryiesire = mysqli_fetch_assoc($rezultatqueryIesire)) {
    ...
    $categorie_produse = $queryiesire['categorie'];
    $imagine_produs = $queryiesire['imagini_produse'];
    $statusProdus = $queryiesire['statusProdus'];
    $g ='g';
    echo " <tr>
    <td>$denumire</td>
    <td>$pret Lei</td>
