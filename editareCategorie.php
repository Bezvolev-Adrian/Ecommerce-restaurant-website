<?php 
        require_once('db.php');
        session_start();
        $id_categorie = $_GET['IDcateg'];
        $query = "SELECT categorie, statusCategorie FROM categorie_produse WHERE id_categorie = $id_categorie;";
        $rezultat = mysqli_query($conexiune, $query);

        while($row =mysqli_fetch_assoc($rezultat)) {
            $categorie = $row['categorie'];
            $status = $row['statusCategorie'];
     }
...
<input type = 'text' placeholder = 'Denumire categorie' class ='email' name = 'categorie' value='$categorie'> 
<input type = 'hidden' class = 'idhidden' value = '$id_categorie'>";    
	...
	$(function(){
		$('.signup').click(function(e){
            ...
            var $reper = $(this).closest(".formsignup");
              ...
              var denumireCategorie = $reper.find(".email").val();
              var statusCategorie = $reper.find(".username").val();
              var idcategorie = $reper.find(".idhidden").val();
				...
				$.ajax({
					type: 'POST',
					url: 'updateCategorie.php',
					data: {denumireCategorie_ajax: denumireCategorie, statusCategorie_ajax: statusCategorie, idcategorie_ajax: idcategorie },
					...
					Swal.fire({ 
                                ...
                                'icon': 'error',
								'title': 'Eroare',
								'text': 'Categoria nu a fost editata!'
