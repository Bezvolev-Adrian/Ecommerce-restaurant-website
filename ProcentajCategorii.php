...
echo "<div style = 'font-size:50px; color:white; position: absolute; left:30%; top:30%;'>Procentaje categorii din total vanzari</div>";
...
$queryTotalMetode = "SELECT COUNT(p.id_categorie) AS total FROM produse as p INNER JOIN produse_comenzi as c ON p.id_produse= c.id_produse INNER JOIN categorie_produse as h ON p.id_categorie = h.id_categorie INNER JOIN comenzi as z ON c.id_comenzi = z.id_comenzi  ";
...
$queryTotalPerCategorie = "SELECT COUNT(p.id_categorie) AS totalPerCategorie, h.categorie FROM produse as p INNER JOIN produse_comenzi as c ON p.id_produse= c.id_produse INNER JOIN categorie_produse as h ON p.id_categorie = h.id_categorie INNER JOIN comenzi as z ON c.id_comenzi = z.id_comenzi GROUP BY p.id_categorie ORDER BY COUNT(p.id_categorie)   ;";
$rezultatPerCategorie = mysqli_query($conexiune, $queryTotalPerCategorie);

while($queryPer = mysqli_fetch_assoc($rezultatPerCategorie)) {

    $totalPerCategorie = $queryPer['totalPerCategorie'];
    $categorie = $queryPer['categorie'];
    $procentaj = ($totalPerCategorie / $total) * 100;
