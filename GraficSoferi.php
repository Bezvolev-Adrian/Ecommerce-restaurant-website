...
echo "<div style = 'font-size:50px; color:white; position: absolute; left:25%; top:30%;'>Activitate soferi: numar comenzi efectuate</div>";
...
$queryTotalPerCategorie = "SELECT COUNT(c.id_soferi) as total, s.nume, s.telefon, s.id_soferi FROM comenzi as c INNER JOIN soferi as s ON c.id_soferi = s.id_soferi WHERE NOT c.id_soferi = 2 GROUP BY c.id_soferi order by COUNT(c.id_soferi) DESC;";
...
foreach ($dataCategorie as $data) { 
    ...
    $arrayTotal[] = $data['total'];
    $arrayCateg[] = $data['nume'];
