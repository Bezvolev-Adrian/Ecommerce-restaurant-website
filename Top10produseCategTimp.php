...
<div style = 'font-size:50px; color:white; position: absolute; left:24%; top:30%;'>Produse vandute pe intervale de timp si categorie </div>
...
<div> 
<form method = 'post' id = 'formCategorie' action = 'Top10produseCategTimpRezultat.php'>
...
<input type = 'submit' name= 'submitDate' value = 'Genereaza raport'style = 'position:absolute; top:65%; left:43%; font-size:25px; background-color: #921f1c; cursor:pointer; padding:7px; border-radius:10px;  color:white; box-shadow: 0px 12px 16px 0px rgba(0,0,0,0.24), 0 17px 50px 0 rgba(0,0,0,0.19); border:none;'>
</form> ";



$queryCategorii = "SELECT id_categorie, categorie FROM categorie_produse;";
$rezultat = mysqli_query($conexiune, $queryCategorii);


echo "<label for= 'selectRang' style= 'color:white; position:relative; left:39%; top: 135px; font-size:25px;' >Categorie</label>
<select id = 'selectRang' form = 'formCategorie' name = 'id_categorie' style = 'position: relative; top: 135px; left:40%; font-size:25px; background-color: #921f1c; cursor:pointer; padding:7px; border-radius:10px;  color:white; box-shadow: 0px 12px 16px 0px rgba(0,0,0,0.24), 0 17px 50px 0 rgba(0,0,0,0.19); border:none;'>";

while($rezQ = mysqli_fetch_assoc($rezultat)) {
    $categorie = $rezQ['categorie'];
    $id_categorie = $rezQ['id_categorie'];
