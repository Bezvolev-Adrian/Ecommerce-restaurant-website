...
echo "<div style = 'font-size:50px; color:white; position: absolute; left:30%; top:30%;'>Popularitate categorii de produse</div>";
echo "<div style= 'position: absolute; top:40%; left:20%; width: 1110px; background-color: white;'>";

echo "<canvas id='myChart' width='auto' height='auto'></canvas>";

echo "</div>";

$queryTotalPerCategorie = "SELECT SUM(c.cantitate) AS totalCantitatePerCateg, h.categorie FROM produse as p INNER JOIN produse_comenzi as c ON p.id_produse= c.id_produse INNER JOIN categorie_produse as h ON p.id_categorie = h.id_categorie GROUP BY h.categorie ORDER BY SUM(c.cantitate) DESC   ;";
...
const myChart = new Chart(ctx, {
    ...
    data: {
        labels: data,
        datasets: [{
            label: 'Numar produse vandute per categorie',
            data: dataTotal,
            backgroundColor: [
                'rgba(255, 99, 132, 0.2)',
                'rgba(54, 162, 235, 0.2)',
