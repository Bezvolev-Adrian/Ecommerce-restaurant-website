...
echo "<div style = 'font-size:50px; color:white; position: absolute; left:40%; top:30%;'> Lista metode de plata </div>";
...
$queryProduseiesire = "SELECT id_metoda_plata, metoda_plata, statusMetode FROM metode_plata";
$rezultatqueryIesire = mysqli_query($conexiune, $queryProduseiesire);
while($queryiesire = mysqli_fetch_assoc($rezultatqueryIesire)) {
    $id_metoda_plata = $queryiesire['id_metoda_plata'];
    $metoda_plata = $queryiesire['metoda_plata'];
    $status = $queryiesire['statusMetode'];


    echo " <tr>
    <td>$id_metoda_plata</td>
    <td>$metoda_plata</td>
    <td>$status</td>
    <td><a href ='editareMetode.php?IDMetoda=$id_metoda_plata' style = 'color:blue;'>Editeaza</a> </td>
